...
## Verification Gate (verify_and_optimize)

- 凡任务包含业务代码修改或 bug 修复：todowrite 清单必须包含「调用 verify_and_optimize（带聚焦 testCommand）→ 应用 bestCode → 复跑该命令确认通过」这一步，完成后才能标记整个任务完成。
- 仅注释 / 格式化 / 重命名 / 纯文档改动可跳过本闸门。
- 插件未通过验证的候选不得应用；返回 critique 中指出的问题需在最终汇报中说明。
<!-- open-mem-context -->

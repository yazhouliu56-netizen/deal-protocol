...
| node:test（`test:oto:units`，103 文件 Glob 自动发现） | 1075 | **base/ammo/adapters 全域**（dispatch/order/money/trust/risk/ai/geo/notify/platform/safe/comm/form + ammo 全表 + gateway 7-provider + lib 扫描/二维码 + pet-boarding 15 例） |
| e2e（playwright `e2e/`） | 4 spec | 基建圈冒烟 |
| e2e-*.mjs 脚本 | 13 | 基建圈回归（verify-prod 13 项 + four-ammos 6 项：5 弹 + 座舱链） |
...
> 阶段验收以「底座确定性 + 弹药可插拔 + 1745 测试基线 + 收敛门禁」为硬门槛。

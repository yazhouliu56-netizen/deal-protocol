...
| node:test（`test:oto:units`，103 文件 Glob 自动发现） | 1090 | **base/ammo/adapters 全域**（dispatch/order/money/trust/risk/ai/geo/notify/platform/safe/comm/form + ammo 全表 + gateway 7-provider + lib 扫描/二维码 + pet-boarding 15 例 + roam-sync 7 例 + llmDirective 5 例 + wave CAS 3 例） |
...
> 阶段验收以「底座确定性 + 弹药可插拔 + 1769 测试基线 + 收敛门禁」为硬门槛。

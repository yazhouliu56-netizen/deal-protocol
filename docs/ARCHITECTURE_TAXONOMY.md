...
| 资产 | 数量 | 归属 |
|------|------|------|
| vitest（根，`test:units`） | 674 | 根侧全域（oto-ui/waves/api + base/ammo 核心域全覆盖 + roam/sync 4 例） |
| node:test（`test:oto:units`，103 文件 Glob 自动发现） | 1079 | **base/ammo/adapters 全域**（dispatch/order/money/trust/risk/ai/geo/notify/platform/safe/comm/form + ammo 全表 + gateway 7-provider + lib 扫描/二维码 + pet-boarding 15 例 + roam-sync 4 例） |
...
> 阶段验收以「底座确定性 + 弹药可插拔 + 1753 测试基线 + 收敛门禁」为硬门槛。

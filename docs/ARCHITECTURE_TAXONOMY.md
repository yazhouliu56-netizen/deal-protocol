...
> 2026-09-05 增补（Microkernel 3.1）：growth 特区 +3 页（/lab、/m20、/f20）+1 API（/api/growth/ammo），`npm run build` 101 路由实证。
**验证基线**：单测 856 全绿（vitest 426 + node:test 430）· Lint exit 0 · tsc 0 错 · 收敛门禁 exit 0
> 2026-09-05 现行基线：**1938/1938 全绿**（vitest 724 + node:test 1214）。历史数字归档备查，不追改。
...
| 资产 | 数量 | 归属 |
|------|------|------|
| vitest（根，`test:units`） | 681 | 根侧全域（oto-ui/waves/api + base/ammo 核心域全覆盖 + roam/sync 4 例 + PublishSheet 真实 Toast 3 例 + ammo/generate 4 例） |
| node:test（`test:oto:units`，106 文件 Glob 自动发现） | 1173 | **base/ammo/adapters 全域**（dispatch/order/money/trust/risk/ai/geo/notify/platform/safe/comm/form + ammo 全表 + gateway 7-provider + lib 扫描/二维码 + pet-boarding 15 例 + roam-sync 11 例 + sync-bus 10 例 + llmDirective 5 例 + wave CAS 3 例 + wave 脱水 4 例 + roamGuard 5 例 + openapi 12 例 + state-crdt 22 例 + directive-schema 27 例） |
...
> 阶段验收以「底座确定性 + 弹药可插拔 + 1854 测试基线 + 收敛门禁」为硬门槛。

...
| 资产 | 数量 | 归属 |
|------|------|------|
| vitest（根，`test:units`） | 672 | 根侧全域（oto-ui/waves/api + base/ammo 核心域全覆盖 + roam/sync 2 例） |
...
> 阶段验收以「底座确定性 + 弹药可插拔 + 1751 测试基线 + 收敛门禁」为硬门槛。

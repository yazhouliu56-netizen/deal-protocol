...
| ADR-0011 语义推荐+BI | ✅ 第三圈库层落地：`base/ai/embed.ts`（中文字 bigram TF 余弦，零依赖）+ `base/ai/bi.ts`（中文指标/类目/时间/TopN 解析 → 聚合，不依赖 LLM）+6 单测（embed 语义排序 / BI 类目收益聚合）+ **宪法 #7 合规**（LLM 介入点评估已记录：延迟/可解释性/规模/断供四点理由，真实 embedding API 与 LLM 意图改写留后续 ADR）。**⚠️ 库储备：** 生产消费方（推荐/BI 界面）未接线 | 2026-08-13 |
| ADR-0012 鉴真+签章/保险 | ✅ 第三/四圈库层落地：`base/ai/forgery.ts`（五信号加权鉴真：EXIF/文件名/复用/时间/比例 + 文本重复 + LLM 复核降级链）+ `base/platform/signInsure.ts`（djb2 签章验签防篡改 + 履约险投保/理赔幂等壳）+9 单测。**⚠️ 库储备：** 生产消费方未接线 | 2026-08-13 |
| ADR-0013 危机+脱敏/遗忘 | ✅ 第五圈库层落地：`base/safe/crisis.ts`（级别 0-3 + EPA 通知递增：联系人→平台值班→警方通道 + 去重/处置闭环/SMS 模板）+ `base/safe/privacy.ts`（五类掩码 + 遗忘权请求幂等 + 按域匿名化）+5 单测。**⚠️ 库储备：** 生产消费方未接线 | 2026-08-13 |

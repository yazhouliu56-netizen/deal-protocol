...
| 2026-08-14 | 待提交 | **Meetup 吸收项 ③ review 低分强制解释（≤3 星必填理由）**：ReviewSection 提交拦截——最终分 = 三维均值（punctual/attitude/professional），`explanationRequired`（≤3 星且正文空/不含理由触发词）→ 红框警示「低分评价必须填写理由（防恶意差评）」拦截、零副作用；星标变化即重评（改高/补文后自动解除警示）；浏览器双身份全链路实测（发布→接单→申报→验收→打 1 星→无理由提交被拦（不入库/面板保持/红框警示）→填理由「迟到 25 分钟且工具没带齐」→入库 ★1 + reviewedBy 幂等位 + UI「已评价 ★1」）；PROJECT_STATUS backlog ③勾销 | |

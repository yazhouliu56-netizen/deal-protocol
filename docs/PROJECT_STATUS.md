...
| 2026-08-15 | Phase 6（本批次，见 git log） | **单仓融合 Phase 6 竣工：子目录废弃、文档统一、终态归档**：① **`oto-spatial-web/` 完全移除**（git rm 68 tracked + 残留 untracked 清理）：12 个 e2e-mjs + verify-prod/restart-prod/export-sofa-glb/generate-vapid 迁移根 `scripts/`，`test:oto:units` node:test 清单（425）迁根 package.json 直跑（57 文件已在根 src/），`dev:all` 单进程化（:3000，子 3001 移除）；workspaces 收敛 `["mobile","packages/*"]`，package-lock 用 Node 精确剔除 oto-spatial-web 残留块；tsconfig 删 `@oto/*` 别名与 exclude、vitest/eslint exclude 同步清理；**`.env.local` 已核对 6 键全 MATCH**（VAPID_PRIVATE/PUBLIC/SUBJECT + SUPABASE_SERVICE_ROLE + NEXT_PUBLIC_*），密钥资产无丢失；e2e 脚本 BASE=:3000 直指根应用，CI/workflow 语义不变（子 CI 与其等价的根 CI 并存无需迁移）② **README.md 重写**为单体架构定位（协议前端 + /oto 5 屏 + 5-provider Gateway + PWA 真推 + 851 测试/12 E2E/105+ API/47+5 屏/100+ 组件指标 + 新目录树 base/ammo/store/oto）；③ **MERGE_PROGRESS.md 归档**：全部 Phase [x] + 【ALL COMPLETED · 深度融合圆满完成】+ 终态基线登记；④ 终态回归：根 tsc 0 错 + `npm test` 851/851（vitest 426 + node:test 425）+ `next build` exit 0 + 收敛门禁 exit 0 | |

...
| 2026-08-13 | 未推 | **库储备接线批次 B（ADR-0012/0014/0015 消费方补齐，第四+六+一圈）**：① 熔断器接 clusterPushes（fail≥3 → open 熔断、30s 冷却后半开探测、熔断期本地 mock 抽取、回执持久化）② sendIm 弱网离线缓冲（离线入队 + online 自动重放 + ContactCard 队列提示条/手动重发）③ 数据湖哈希链存证（验收/开争议/争议终局 append，AdminPanel 存证块全链校验 + 最近事件）④ 验收签章（acceptFulfilment 生成 signedDoc，争议面板验签展示）⑤ DynamicFormView 通用渲染端 + ProfilePage 紧急联系人 schema 表单（SOS 发起读已存联系人）；store 新增 circuitBreaker/offlineQueue/lake/signedDocs（transport 本地缓冲语义 next 优先防哈希链断裂、partialize 持久化）；geoAdapter 留 mobile 融合、AB/degrades 无真实场景不硬造；→ 单测 389 全绿、tsc/lint/build 通过 | |

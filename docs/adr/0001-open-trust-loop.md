...
## Addendum（2026-08-06）: 发布费独立化 + 老数据取消 B 方案

- **发布费独立**：每日免费发布 `FREE_PUBLISH_PER_DAY=3` 次，超出每次收
  `PUBLISH_FEE=1`。发布费与单子金额完全独立（`PayKind` 区分 `seat` / `publish-fee`），
  一经支付**不退**（发布动作已发生；免费次数取消也不返还）。提交即扣：提交时消耗
  免费次数；用完则随单子一起在 PaySheet 两笔并列支付。退款（成团失败/分级取消）
  一律跳过 publish-fee 订单。
- **取消 B 方案（无 startsAt 老数据）**：无法计算 lead time 时按「是否已成局」判定——
  未成局且无人拼位 = 零成本取消 → 全额退 seat；已有人拼位/已成局 = 不退
  （避免被人爽约产生成本）。原方案「一律不退」过严，已改。

## Alternatives Rejected

- 取消返还会退还免费次数/发布费：违「发布动作已发生，成本不可逆」原则，被拒。

...
| 2026-08-14 | 待提交 | ammo 四表 → base 引擎接线批次：broadcastMatches/passesHardFilter/requiresVerification 增 rule 参数（DEFAULT_BROADCAST_RULE 等价原常量）；riskOf/extraLogin 增 params 参数（DEFAULT_ROAM_PARAMS 等价原阈值）；WaveFeed/useWaveStore/useRoamStore/RoamGuardPanel/AdminPanel 从 ammo 表取值注入；BiddingSandboxCard reserve 地板价；PublishSheet SOP 预填 + 定价建议 + 有效期档表驱动；ammo/sop 补「羽毛球约局」别名 | #3 先配表后写码 / #4 弹药可插拔 / #5 引信跟弹药走 | 写死权重/阈值/默认值收敛为 ammo 表驱动（参数注入式，base 保持零 import 可 node:test 直跑）；契约修订仅增补可选参数，未改既有语义 |
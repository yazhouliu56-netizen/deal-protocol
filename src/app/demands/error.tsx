...
export default function DemandsError({ error, reset }: { error: Error; reset: () => void }) {
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-zinc-950 p-6 text-center">
      ...
      <h2 className="text-xl font-bold text-zinc-100 mb-2">需求页面加载异常</h2>
      <p className="text-sm text-zinc-400 mb-6 max-w-md">{error.message || "页面数据加载失败，请稍后重试"}</p>
      <button
        onClick={reset}

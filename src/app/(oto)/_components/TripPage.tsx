...
export default function TripPage({ proofShots = [], onProofShot }: { proofShots?: ArbitrationPhotoEvidence[]; onProofShot?: (shot: ArbitrationPhotoEvidence) => void }) {
  ...
  return (
    <div className="pointer-events-auto">
      ...
      {!activeOrder && (
        <div className="mt-2 bg-white border border-[#e5e5e5] shadow-sm rounded-3xl p-6 flex flex-col items-center text-center" data-testid="trip-empty-state">
      ...
      {bookings.length > 0 && (
        <div className="mt-3">
          <span className="text-xs font-semibold text-[#afafaf] mb-2 flex items-center gap-1.5"><span className="w-1 h-3 rounded-full bg-linear-to-b from-brandCyan to-brandPurple" /> 我的预订</span>
          <div className="flex justify-between items-baseline mb-2"><p className="text-xs text-[#afafaf]">共 {bookings.length} 个真实预订 · 点按进入订单详情</p><span className="text-xs px-2 py-0.5 rounded-full bg-brandCyan/15 border border-brandCyan/40 text-brandCyan font-bold">履约中枢</span></div>

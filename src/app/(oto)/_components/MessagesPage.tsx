  ...
  return (
    <div className="pointer-events-auto">
      ...
      {convos.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">{convos.map((c) => { const myNumber = dialInNumber(c.session, me); return (<DuoCardShell key={c.session.waveId} className="shadow-sm p-3"><div className="flex items-center gap-2.5 mb-2"><div className="w-9 h-9 rounded-xl bg-white border-2 border-[var(--color-duo-swan)] flex items-center justify-center text-base shrink-0 shadow-sm">{CATEGORY_EMOJI[c.wave?.basics.category ?? ""] ?? "🎟️"}</div><div className="flex-1 min-w-0"><p className="text-[12px] font-bold text-[var(--color-duo-eel)] truncate">{c.wave?.basics.category ?? "订单会话"}</p><p className="text-xs text-[var(--color-duo-wolf)] truncate font-mono">{maskNumber(myNumber)} · {c.live ? `${minutesLeft(c.session, now)} 分钟后失效` : "会话已过期"}</p></div>{c.unreadForMe > 0 && <span className="px-1.5 py-0.5 rounded-full bg-[var(--color-duo-green)] text-white text-xs font-extrabold font-tabular border-2 border-white shadow-sm">{c.unreadForMe}</span>}</div>{c.last && <p className="text-xs text-[var(--color-duo-wolf)] truncate mb-1.5 bg-[var(--color-duo-polar)] rounded-xl px-2.5 py-2 border border-[var(--color-duo-swan)]">{c.last.fromId === me ? "我：" : "对方："}{c.last.text}</p>}<ContactCard waveId={c.session.waveId} peerId={c.peerId} /></DuoCardShell>); })}</div>
      )}
    </div>
  );
}

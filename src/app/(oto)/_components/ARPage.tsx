...
export default function ARPage({ proofShots, onProofShot }: { proofShots: ArbitrationPhotoEvidence[]; onProofShot: (shot: ArbitrationPhotoEvidence) => void }) {
  ...
  return (
    <div className="flex-1 w-full flex flex-col items-center relative min-h-0 pointer-events-auto">
      <div className="flex items-center gap-1.5 z-20 shrink-0 mb-2 pointer-events-auto">{([{ id: "scene", label: "📍 附近探索", hint: "看看附近可撮合的服务" }, { id: "preview", label: "✨ 体验预览", hint: "3D 模型预览" }] as const).map((m) => (<button key={m.id} onClick={() => { setMode(m.id); setActivePoint(null); }} className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${mode === m.id ? "bg-[var(--color-duo-green)] border-b-2 border-[var(--color-duo-green-dark)] text-white" : "bg-white border-2 border-[var(--color-duo-swan)] text-[var(--color-duo-wolf)] hover:text-[var(--color-duo-eel)]"}`}>{m.label}</button>))}</div>
      <div className="flex-1 min-h-0 w-full flex items-center justify-center relative">
        {mode === "scene" ? (
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute inset-0 scene-grid" />
            <div className="absolute inset-x-6 top-4 bottom-6 rounded-2xl border-2 border-[var(--color-duo-swan)] pointer-events-none"><span className="absolute -top-[7px] left-3 px-1 text-xs tracking-[0.3em] text-white bg-black/30 rounded">服务探索区</span></div>
            <div className="absolute inset-0 pointer-events-auto">{AR_SCENE_POINTS.map((p) => (<button key={p.id} onClick={() => setActivePoint(p)} aria-label={p.name} className="absolute flex flex-col items-center group active:scale-95 transition-transform" style={{ left: p.x, top: p.y, transform: "translate(-50%, -50%)" }}><span className={`w-3.5 h-3.5 rounded-full ${activePoint?.id === p.id ? "bg-[var(--color-duo-green)]" : "bg-[var(--color-duo-blue)]"} animate-ping-once`} /><span className="mt-1.5 px-2 py-1 rounded-full bg-white border-2 border-[var(--color-duo-swan)] text-xs font-bold text-[var(--color-duo-eel)] whitespace-nowrap">{p.emoji} {p.name}</span><span className="mt-0.5 text-xs text-[var(--color-duo-hare)]">{p.distance}</span></button>))}</div>
          </div>
        ) : (
          <>
      ...
      <div className="w-full z-20 space-y-3 shrink-0 pointer-events-auto">
        {mode === "scene" ? (
          <>{activePoint ? (<motion.div key={activePoint.id} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }} className="bg-white border-2 border-[var(--color-duo-swan)] p-4 rounded-3xl animate-float-slow"><div className="flex items-center gap-3"><div className="w-11 h-11 rounded-2xl bg-white border-2 border-[var(--color-duo-swan)] flex items-center justify-center text-xl shrink-0">{activePoint.emoji}</div><div className="flex-1 min-w-0"><div className="flex items-center gap-1.5"><h3 className="font-bold text-[14px] truncate">{activePoint.name}</h3><span className="flex items-center gap-0.5 text-xs font-semibold text-yellow-400 shrink-0"><Star size={10} className="fill-yellow-400" />{activePoint.rating}</span></div><p className="text-xs text-[var(--color-duo-wolf)] mt-0.5 truncate">{activePoint.meta}</p><p className="text-xs text-brandCyan font-bold mt-0.5">{activePoint.price} · 距你 {activePoint.distance}</p></div></div><DuoButton onClick={() => goMatch(activePoint.draft)} variant="primary" size="md" fullWidth className="mt-3">找 AI 撮合 →</DuoButton></motion.div>) : (<div className="bg-white border-2 border-[var(--color-duo-swan)] px-4 py-3 rounded-2xl flex items-center gap-2 animate-float-slow"><p className="text-xs text-[var(--color-duo-wolf)]">点击光点探索附近可撮合服务</p></div>)}</>
        ) : (
          <>
            <AnimatePresence>{showInfo && (<motion.div initial={{ ...RISE_10.initial, height: 0 }} animate={{ ...RISE_10.animate, height: "auto" }} exit={{ opacity: 0, y: 10, height: 0 }} transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }} className="overflow-hidden"><GlassCard className="p-3.5 rounded-2xl"><p className="text-xs text-[var(--color-duo-eel)] leading-relaxed">{selectedExperience.description}</p></GlassCard></motion.div>)}</AnimatePresence>
            <div className="bg-white border-2 border-[var(--color-duo-swan)] p-4 rounded-3xl animate-float-slow"><div className="flex justify-between items-start mb-1"><div><h3 className="font-bold text-base">{selectedExperience.title} · {selectedExperience.location}</h3><p className="text-xs text-[var(--color-duo-wolf)] mt-0.5">{selectedExperience.subtitle} · {selectedExperience.price}</p></div><span className="flex items-center gap-1 text-xs font-semibold text-yellow-400 shrink-0"><Star size={11} className="fill-yellow-400" /> {selectedExperience.rating}</span></div><div className="flex gap-3"><DuoButton onClick={() => goMatch(`想预约 ${selectedExperience.title} · ${selectedExperience.location} 的体验工作坊`)} variant="primary" size="sm" className="flex-1">预约工作坊</DuoButton><DuoButton onClick={() => toggleCart(selectedExperience.id)} variant="outline" size="sm" sound="click" className="flex-1">{addedToCart ? "已加入" : "心愿单"}</DuoButton></div></div>
          </>
        )}
      </div>
      {photoOpen && (<><motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-40 bg-black/50" onClick={() => setPhotoOpen(false)} /><motion.div initial={{ y: 60, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ type: "spring", stiffness: 300, damping: 28 }} className="fixed inset-x-3 bottom-24 z-50 bg-white border border-[var(--color-duo-swan)] shadow-sm rounded-3xl p-4 max-h-[72vh] overflow-y-auto no-scrollbar"><div className="flex items-center justify-between mb-3"><h3 className="text-[13px] font-extrabold flex items-center gap-1.5"><Camera size={13} className="text-brandCyan" /> 拍照存证 · 时间地点水印</h3><button onClick={() => setPhotoOpen(false)} aria-label="关闭相机" className="text-[var(--color-duo-hare)] hover:text-[var(--color-duo-eel)]">✕</button></div>{proofShots.length > 0 && <p className="text-xs text-[var(--color-duo-green-dark)] mb-2">✅ 当前已存证 {proofShots.length} 张（含水印 + SHA-256 指纹）</p>}<ProofCamera orderNo={cameraOrderNo} geo={{ lat: 31.2304, lng: 121.4737, accuracyMeters: 25 }} onCaptured={(result) => { onProofShot({ photo: result.dataUrl, aiNote: `水印存证 · 时间地点注入 · 哈希 ${result.sha256.slice(0, 8)}` }); setPhotoOpen(false); toast("✅ 存证照片已生成 · 时间地点水印 + 哈希指纹已记录", "success"); }} /></motion.div></>)}
    </div>
  );
}

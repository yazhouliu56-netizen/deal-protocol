 ...
 *
 * 单一方程（整数分，守恒硬锁）：
 *   providerNet + qualityFee + channelFee ≡ totalCents
...
/** 客观轨 85 : 主观三勾各 5（最大余数法分配序，固定索引打破平局）。 */
const TYPE1_RATIOS = [85, 5, 5, 5] as const;
...
export interface Type1Settlement {
  ...
  /** 服务者实得（base + released − channelFee）。 */
 ...
 */
export function splitType1Shares(totalCents: number): {
  baseCents: number;
 ...
 */export function settleType1(
  ...
  const shares = splitType1Shares(totalCents);
  ...
  if (providerNetCents + qualityFeeCents + channelFeeCents !== totalCents) {
    throw new Type1SettlementError(
      "INVALID_TOTAL",
      `资金守恒破坏：${providerNetCents}+${qualityFeeCents}+${channelFeeCents} !== ${totalCents}`,
    );
  }
  return {
    totalCents,

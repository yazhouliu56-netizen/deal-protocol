...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-zinc-200/60 bg-white/80 backdrop-blur-xl supports-[backdrop-filter]:bg-white/60 dark:border-zinc-800/60 dark:bg-zinc-950/80 dark:supports-[backdrop-filter]:bg-zinc-950/60">
      <div className="mx-auto flex h-14 max-w-7xl items-center justify-between px-6 sm:px-8 lg:px-10">
        ...
        <div className="hidden items-center gap-2 md:flex">
          {loading ? (
            ...
            <>
              <NotificationBell />
              <DropdownMenu>
                <DropdownMenuTrigger className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-sm font-medium text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900 outline-none transition-colors dark:text-zinc-400 dark:hover:bg-zinc-800 dark:hover:text-zinc-100">
                  <User className="size-4" />
                  <span className="max-w-[100px] truncate">{session.name || session.email}</span>
                  <ChevronDown className="size-3 opacity-50" />
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-44">
                  <div className="px-2.5 py-2 text-xs text-zinc-500 border-b border-zinc-200 mb-1 dark:text-zinc-400 dark:border-zinc-800">
                    {session.email}
                  </div>
        ...
        <button
          type="button"
          className="flex h-11 w-11 items-center justify-center rounded-lg md:hidden hover:bg-zinc-100 transition-colors dark:hover:bg-zinc-800"
      ...
      {mobileOpen && (
        <div className="border-t border-zinc-200/60 md:hidden animate-in dark:border-zinc-800/60 touch-manipulation">
          <nav className="flex flex-col gap-px px-6 py-3">
            <Link href="/" onClick={() => setMobileOpen(false)} className="touch-target flex items-center rounded-lg px-3 text-sm font-medium text-zinc-900 hover:bg-zinc-100 active:bg-zinc-200 dark:text-zinc-100 dark:hover:bg-zinc-800 dark:active:bg-zinc-700">首页</Link>
            {isLoggedIn && <Link href="/demands/new" onClick={() => setMobileOpen(false)} className="touch-target flex items-center rounded-lg px-3 text-sm text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900 active:bg-zinc-200 dark:text-zinc-400 dark:hover:bg-zinc-800 dark:hover:text-zinc-100 dark:active:bg-zinc-700">发布需求</Link>}
            {isLoggedIn && <Link href="/dashboard" onClick={() => setMobileOpen(false)} className="touch-target flex items-center rounded-lg px-3 text-sm text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900 active:bg-zinc-200 dark:text-zinc-400 dark:hover:bg-zinc-800 dark:hover:text-zinc-100 dark:active:bg-zinc-700">我的订单</Link>}
            {isProvider && <Link href="/provider/incoming" onClick={() => setMobileOpen(false)} className="touch-target flex items-center rounded-lg px-3 text-sm text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900 active:bg-zinc-200 dark:text-zinc-400 dark:hover:bg-zinc-800 dark:hover:text-zinc-100 dark:active:bg-zinc-700">待接需求</Link>}
            ...
            <hr className="my-2 border-zinc-200/60 dark:border-zinc-800/60" />
            {loading ? (
              <div className="h-11 animate-pulse rounded-lg bg-zinc-200 dark:bg-zinc-800" />
            ) : session ? (
              <>
                <div className="touch-target flex items-center gap-2 px-3 text-sm font-medium text-zinc-900 dark:text-zinc-100">
                  <User className="size-4" />
                  <span className="truncate">{session.name || session.email}</span>
                </div>
                <Link href="/profile" onClick={() => setMobileOpen(false)} className="touch-target flex items-center rounded-lg px-3 text-sm text-zinc-500 hover:bg-zinc-100 active:bg-zinc-200 dark:text-zinc-400 dark:hover:bg-zinc-800 dark:active:bg-zinc-700">个人资料</Link>

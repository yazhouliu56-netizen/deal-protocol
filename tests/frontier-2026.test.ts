...
// ============================================================
// Mechanism 2: AI Bi-directional Negotiation Engine
// ============================================================
describe('AINegotiator', () => {
  it('proposeCounterOffer exports correct function', async () => {
    const { proposeCounterOffer } = await import('@/lib/ai-negotiator')
    expect(proposeCounterOffer).toBeDefined()
  })
  ...
  it('returns fallback midpoint when API unavailable', async () => {
    ...
    expect(result.scopeAdjustments.length).toBeGreaterThanOrEqual(1)
    expect(result.estimatedProbability).toBeGreaterThanOrEqual(0)
  })
  ...
  it('midpoint is correct average', () => {
    const budget = 200
    const expected = 250
    const mid = Math.round((budget + expected) / 2)
    expect(mid).toBe(225)
  })
})

...
// ============================================================
// P0-8: Milestone Checkpoint Tests (submit / confirm / batch timeout)
// ============================================================
describe('MilestoneCheckpoint', () => {
  beforeEach(() => {
    vi.resetModules()
  })

  it('submitMilestoneCheckpoint sets status to submitted and calculates auto_confirm_at', async () => {
    const { submitMilestoneCheckpoint } = await import('@/lib/milestone-escrow')
    const mockSupabase = {
      from: vi.fn(() => ({
    ...
    expect(result.autoConfirmAt).toBeTruthy()

    const expectedMs = Date.now() + 24 * 3600 * 1000
    const actualMs = new Date(result.autoConfirmAt).getTime()
    expect(Math.abs(actualMs - expectedMs)).toBeLessThan(1000)

    __resetServiceClient()
  })

  it('submitMilestoneCheckpoint throws when db update fails', async () => {
    const { submitMilestoneCheckpoint } = await import('@/lib/milestone-escrow')
    const mockSupabase = {
      from: vi.fn(() => ({
        update: vi.fn(() => ({
  ...
  it('confirmMilestoneCheckpoint sets status to completed and clears auto_confirm_at', async () => {
  ...
  it('processExpiredCheckpoints returns { processedCount, errors } shape', async () => {
    const { processExpiredCheckpoints } = await import('@/lib/milestone-escrow')
    const mockSupabase = {
      from: vi.fn(() => ({
        select: vi.fn().mockReturnThis(),
        eq: vi.fn().mockReturnThis(),
  ...
  it('processExpiredCheckpoints processes multiple expired checkpoints', async () => {
    const { processExpiredCheckpoints } = await import('@/lib/milestone-escrow')
    const expiredRows = [
  ...
  it('processExpiredCheckpoints reports partial errors', async () => {
    const { processExpiredCheckpoints } = await import('@/lib/milestone-escrow')
    let callCount = 0
    const mockSupabase = {
      from: vi.fn(() => ({
        ...
        eq: vi.fn(() => {
          callCount++
          if (callCount === 1) return { lte: vi.fn().mockResolvedValue({ data: [{ id: 'c1', contract_id: 'ct-1', title: 'T', amount: 50 }], error: null }) }
          return Promise.resolve({ error: new Error('conflict') })
        }),
        update: vi.fn().mockReturnThis(),
        lte: vi.fn(),
      })),
